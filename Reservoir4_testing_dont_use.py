...
class Reservoir:
    ...
    def regulated_release(self, param, h):
        ...
        hmax = self.pool_elev #added 12/9
        hmin = h0
        mef = param['mef']
        h1 = param['h1']
        h2 = param['h2']
        ...
        L1 = mef + m1 * (h - h1) # save water for future??
        # L2 = mef + m2 * (h - h2)
        r = np.maximum (L1, mef)
        # r = np.maximum(np.minimum(L1, mef), L2)  # Release
        ...
        r = np.where(h<= hmin,0,r)
        r = np.where(h>=hmax, beta*(h-h0)**alfa,r)
        # r = np.where(h >= hmax, beta * (h - h0) ** alfa, r)  ##added 12/9
        # r = np.minimum(natural_flow, r)  # Do not exceed natural flow ever
        r = np.maximum(r, 0)  # Release cannot be negative
        natural_flow = beta * (h-h0)**alfa
        r = np.where(r>natural_flow, natural_flow,r)
        r = np.where(h<h0,0,r)
