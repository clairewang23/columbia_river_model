...
# CONSTANTS: for hydropower calculation
...
class Reservoir:
    ...
    def __init__(self, SA, capacity, tail_elev, pool_elev, bottom_elev, pc, alfa, beta):
    ...
    def set_params(self, alfa, beta):
        '''
        A function to manually set the alfa and beta parameters for the reservoir natural
        release function.

        Parameters:
        - alfa (float): Parameter for natural lake model 'alfa'
        - beta (float): Parameter for natural lake model 'beta'
        '''
        self.alfa = alfa
        self.beta = beta
    ...
    def simulation_nat_lake(self, keep, h_in, n):
        ...
        Parameters:
        - keep (int/boolean): whether the dam is kept or not. (1 = dam remains, 0 = no dam)
        - h_in (float): Initial lake level (m)
        - n (array-like): Net inflows trajectory (in m^3/s)
        
        Returns:
        - tuple: (lake storage trajectory (m^3), lake level trajectory (m), release trajectory (m^3/s))
        """
    ...
    def regulated_release(self, param, h):
        ...
        Parameters:
        - param (dict): Lake model parameters with 'mef', 'h1' and 'm' keys
        ...
        # Constraints
        r = np.where(h <= h0, 0, r)  # No flow possible if h is below tailwater elevation

        # Clip the base to avoid raising negative numbers to fractional powers
        base = np.maximum(h - h0, 0)
        natural_flow = beta * (base ** alfa)

        r = np.minimum(natural_flow, r)  # Do not exceed natural flow
    ...
    def simulation_reg_lake(self, keep, param, h_in, n):
        ...
        - h_in (float): Initial lake level (m)
        - n (array-like): Net inflows trajectory (m^3/s)
    ...
    def simulate_head(self, height):
        '''
        Calculate the head for the turbine.

        Parameters:
        - height (float or numpy array): water elevation (m)

        Returns:
        - head (float or numpy array): turbine hydraulic head (m)
        '''
    ...
    def simulate_hydropower(self, head, flow, keep):
        '''
        Simulate the daily hydropower production for a dam.

        Parameters:
        - head (float/np array): turbine hydraulic head (m)
        - flow (float/np array): reservoir outflow (m^3/s)
        - keep (int/boolean): whether the dam is kept or not. (1 = dam remains, 0 = no dam)

        Returns:
        - energy (float/np array): energy generated by the dam in one day (kWh)
        '''
    ...
    def calc_avg_annual_hydro(self, date, hydro):
        '''
        Calculate the average annual hydropower generated by the dam.

        Parameters:
        - date (np array of datetime objects): datetime objects for the dataset
        - hydro (np array of floats): hydropower generation timeseries corresponding to the dates

        Returns:
        - avg_annual_hydro (float): average annual hydropower generation (kWh)
        '''
    ...
    def simulate(self, keep, initial_height, param, datetime, prev_out, tributary):
        '''
        Simulate the reservoir behavior.

        Parameters:
        - keep (int/boolean): whether the dam is kept or not. (1 = dam remains, 0 = no dam)
        - initial_height (float): initial water elevation in reservoir (m)
        - param (dict): Parameters with lake surface and model settings 
        - datetime: array of datetime values for each datapoint
        - prev_out: array of outflow values from the previous reservoir upstream (m^3/s)
        - tributary: array of tributary flow values (m^3/s)

        Returns:
        - outflow (array): reservoir release timeseries (m^3/s)
        - avg_hydro (float): average annual hydropower generated (kWh)
        - height (array): reservoir water elevation timeseries (m)
        '''
